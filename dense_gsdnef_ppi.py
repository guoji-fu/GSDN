...
class Net(torch.nn.Module):
    def __init__(self,
        ...
        self.conv2 = DenseGSDNEFConv(self.hidden_num, self.hidden_num, self.alpha, self.K)
        self.lin2 = torch.nn.Linear(self.hidden_num, self.hidden_num)
        self.conv3 = DenseGSDNEFConv(self.hidden_num, self.out_channels, self.alpha, self.K)
        self.lin3 = torch.nn.Linear(self.hidden_num, self.out_channels)

    def forward(self, x, adj):
        x = F.relu(self.conv1(x, adj) + self.lin1(x))
        x = F.relu(self.conv2(x, adj) + self.lin2(x))
...
def train(model, optimizer, train_loader, loss_op, device):
    ...
    for data in train_loader:
        ...
        data = data.to(device)
        optimizer.zero_grad()
        loss = loss_op(model(data.x, data.adj), data.y)
...
def test(model, loader, device):
    ...
    for data in loader:
        ...
        ys.append(data.y)
        with torch.no_grad():
            out = model(data.x.to(device), data.adj.to(device))
